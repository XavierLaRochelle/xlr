...
***À noter pour le TP :***  
Même si les trois méthode sont présenté dans ce guide, vous aurez uniquement besoin d'utiliser le test Shapiro-Wilk pour votre TP.
...
Lorsqu'on vérifie la normalité à l'aide de ces indices, on cherche généralement à ce qu'ils se trouvent à l'intérieur d'un intervalle de ±1. Vous pouvez les obtenir à l'aide de la fonction `descritives()` du package `jmv` :
```{r}
library(jmv)
descriptives(df, emotionPositive_Post,
             kurt = TRUE,
             skew = TRUE)
```
Dans ce cas-ci, on observe que l'échantillon présente une faible asymétrie négative ($\text{Skew} ≈ -0,06$) et une forme légèrement platykurtique ($\text{Kurt} ≈ -0,12$). Toutefois, puisque ces indices demeurent à l'intérieur d'un intervalle de ±1, on peut conclure que notre échantillon respecte le postulat de normalité.
