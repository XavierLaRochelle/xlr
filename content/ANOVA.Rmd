...
```{r, message=FALSE}
library(car)
leveneTest(Petal.Length ~ Species, data = df)
```
...
## Faire le test
```{r}
kruskal.test(Petal.Length ~ Species, data = df)
```

On peut encore une fois conclure que la longueur moyennes des pétales diffère significativement selon l'espèce de fleur ($\chi^2(2) = 130,4; p < ,05$). 
