...
Pour parvenir à rédiger vous-même un script RMarkdown, vous devrez lire attentivement les sections principales (1, 2 et 3) et la section 3.1. 

Vous pouvez vous permettre de lire un peu moins attentivement les sous-sections suivantes : 1.1 à 1.3, 2.1 à 2.5. 

Le format de fichier RMarkdown est extrêmement flexible et, pour s'y retrouver, il faut savoir distinguer ses sous-composantes. Un script RMarkdown peut-être subdivisé en trois types de contenus: l'**entête YAML**, le **texte** et les **blocs de code** (`chunks`).

---

# Entête YAML
Une entête (donc située au tout début de votre script) de métadonnées facultatives entourée de 3 tirets (`---`). 
